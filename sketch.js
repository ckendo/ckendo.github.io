...
let sayings = [];
// let sayings = ["we want a sumptuous drizzling please",
...
// Google sheets
let data;
var url = "https://docs.google.com/spreadsheets/d/e/2PACX-1vThQ5R33TbQG2oh2hN4VN71BjQ21Y-kF5WqwvDoqqoeqKtnSNSR5VK7M4zNyGwedAs1IxPDpWLgkUHX/pub?gid=1515489014&single=true&output=csv"
...
function drawAudio() {
	...
	for (var i = 0; i < 10; i ++) {
		ellipse(0, 30, h/4, h*4);
		rotate(PI/10);// +radians(frameCount)/10)
	}
	pop()
...
function drawBlob() {
	...
	var vol = mic.getLevel();
	var d = map(vol, 0, 1, height*0.8, 0);
...
function variableRectangle(x, y, px, py) {
	let speed = abs(x - px) + abs(y - py);
	stroke(speed);
	rect(x, y, speed/10, speed/10);
}
